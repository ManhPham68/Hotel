                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            @foreach($lsGuest as $guest)
                                <tr>
                                    <td>
                                        <input type="checkbox" name="email" value="{{$guest->email}}">
                                    </td>
                                    <td> {{$guest->title}} </td>
                                    <td> {{$guest->name}} </td>
                                    <td> {{$guest->age}} </td>
                                    ...
                                    <td> {{$guest->email}} </td>
                                    <td> {{$guest->country}} </td>
                                    <td> <img src="{{asset($guest->avatar)}}" style="width: 250px;height: 200px;border-radius: 50%"> </td>
                                        ...
                                        <p>
                                            @can('edit_guest')
                                                <a class="btn btn-primary"
                                                   href="{{route('Guest.edit',$guest->id)}}"> Update
                                                </a>
                                            @endcan
                                        </p>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#sendMailNofy').click(function () {
                $('.loader').show();
                var chkChecked = $('input[type="checkbox"][name="email"]:checked');
                var list_email = [];
                if (chkChecked.length > 0) {
                    for (var i = 0; i < chkChecked.length; i++) {
                        list_email.push(chkChecked[i].value);
                    }
                }
                alert(list_email);
                var data = {
